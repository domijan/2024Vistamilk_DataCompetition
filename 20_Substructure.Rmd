...
p <-  pc |>
  augment(medoids) |>
  ggplot(aes(.fittedPC1, .fittedPC2, color = Surface, pch = Surface)) +
geom_point() +
  ylab("PC2 (10%)") +
  xlab("PC1 (80%)") 

p + geom_text_repel(box.padding = 0.9, aes(label = label2)) + theme_classic()
...
grid.arrange(p1, p2, nrow = 1)
 ...
 pc |>
  augment(medoids) |>
  ggplot(aes(.fittedPC7, .fittedPC8, color =(Surface), label = label2)) +
  geom_point() +
  geom_text_repel(box.padding = 0.5)+
  ylab("PC3") +
  xlab("PC1") +
