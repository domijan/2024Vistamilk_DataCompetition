...
rf.pred <- fit_rf |> 
 ...
 #    left_join(truth)
  ...
  pred_table <- tibble(pred = rf.pred$predictions) |>
    mutate(samp = te.samp) |>
    group_by(samp) |>
...
fit_pls <- caret::plsda(
  ...
  predict(dat.te |> select(-Diet))
    ...
    pred_table <- tibble(pred = pred_pls) |>
    mutate(samp = te.samp) |>
    group_by(samp) |>
    summarise(mn.pred =  as.numeric(names(which.max(table(pred))))) |>
    left_join(truth)
...
pred_lasso <- cvfit |> 
  predict(xte, type = "class", s = "lambda.min")
    ...
    pred_table <- tibble(pred = pred_lasso) |>
    mutate(samp = te.samp) |>
    group_by(samp) |>
    summarise(mn.pred =  as.numeric(names(which.max(table(pred))))) |>
    left_join(truth)
...
pred_lda <- pred_lda$class
    ...
    pred_table <- tibble(pred = pred_lda) |>
    mutate(samp = te.samp) |>
    group_by(samp) |>
    summarise(mn.pred =  as.numeric(names(which.max(table(pred))))) |>
    left_join(truth)
...
saveRDS(RMSE, "RMSEclass2.rds")

saveRDS(pred_table, "pred_tableclass2.rds")
